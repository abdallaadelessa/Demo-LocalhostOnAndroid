...
public abstract class NanoHTTPD
{
	...
	public static SSLServerSocketFactory makeSSLSocketFactory(
			InputStream keystoreStream, char[] passphrase)
			throws IOException
	{
		SSLServerSocketFactory res = null;
		try
		{
			Utils.log(KeyStore.getDefaultType());
	...
	public void start() throws IOException
	{
		...
		{
			SSLServerSocket ss = (SSLServerSocket) sslServerSocketFactory
					.createServerSocket();
	...
	public static class Response
	{
		...
		private void send(OutputStream outputStream)
		{
			...
			try
			{
				...
				int pending = data != null ? data.available() : -1; // This is
																	// to
																	// support
																	// partial
																	// sends,
	...
	protected class HTTPSession
	{
		...
		private void parseBody(Map<String, String> files) throws IOException,
				ResponseException
		{
				...
				throws ResponseException
		{
			try
			{
				...
				if (st.hasMoreTokens())
				{
					...
					{
						int p = line.indexOf(':');
						if (p >= 0) headers.put(line.substring(0, p).trim()
								.toLowerCase(), line.substring(p + 1).trim());
						line = in.readLine();
					}
				}
